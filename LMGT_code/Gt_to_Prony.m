...
% Nd = desired logarithmic spacing of tau (points/decade) [l..5], default=2
% example: t=logspace(-3,2)'; G=l+9*exp(-t.^0.5); Gt_to_Prony(G,t,2);

% PRONY FITTING
Ge=min(Gtt)*0.98;                           % rubbery plateau
logt0=log10(min(tt)); logt1=log10(max(tt));
tau = 10.^(logt0:1/Nd:logt1)';              % chosen Prony times
% xx = kron(ww,tau'); X = xx.^2./(l+xx.^2); %   dynamic data (Gw,w)
